...
register({
  ...
  pathnameRegex: /^\/(?:$|reel\/[^/]+\/?|(?:[^/]+\/)?posts\/[^/]+\/?|groups\/[^/]+\/posts\/[^/]+\/?|[^/]+\/videos\/[^/]+\/?|watch\/?|(?:permalink|story)\.php)$/,
  ...
  async extract() {
    ...
    const kind = /\/reel\//.test(window.location.pathname) ? 'reel'
      : /\/videos?\//.test(window.location.pathname) || window.location.pathname === '/watch/' ? 'video'
      : isFeed ? 'active feed item' : 'post';
    const title = post.author ? `Facebook ${kind} by ${post.author}` : `Facebook ${kind}`;
...
function getPostId(): string {
  ...
  return path.match(/\/(?:posts|reel|videos)\/([^/?]+)/)?.[1]
...
function extractMedia(scope: ParentNode): string[] {
  const media: string[] = [];
  scope.querySelectorAll<HTMLImageElement>('img[alt]').forEach((image) => {
    const alt = clean(image.alt);
    if (alt && !/profile picture|avatar|emoji|sticker/i.test(alt)) media.push(alt);
  });
